...
def _run_lib_tests(port, label, token, approve):
    ...
    # 12g. Route registration constraints — reserved prefixes reject at
    # activation, shadowing warns but allows. Pre-merge red team 2026-04-21:
    # a plugin with ROUTES=[''] would intercept every request and lock the
    # operator out of HTTP recovery. /lib/* shadow blocks state-transition.
    # /bin/* scrambles alias-strip dispatch.
    def _try_activate(wname, routes_literal):
        """Install a test plugin and attempt activation. Returns
        (activate_status_code, detail_string)."""
        http_method(port, f"/lib/{wname}", method="DELETE", token=approve)
        src = (f"ROUTES = {routes_literal}\nAUTH = 'none'\n"
               f"async def handle(m, b, p): return {{'ok': True}}\n")
        http_method(port, f"/lib/{wname}", method="PUT", body=src, token=token)
        st, body = http_method(port, f"/lib/{wname}/state", method="PUT",
                               body="active", basic_auth=approve)
        return st, body

    # Empty string wildcard — the bricking case.
    st, body = _try_activate("reserved-empty", "['']")
    test(f"{label} lib: route '' rejected at activation",
         st == 422, f"status={st} body={body[:120]}")
    test(f"{label} lib: route '' — error mentions reserved/non-empty",
         "non-empty" in body or "reserved" in body,
         f"body={body[:120]}")

    # /lib — blocks state transition endpoint.
    st, body = _try_activate("reserved-lib", "['/lib']")
    test(f"{label} lib: route '/lib' rejected (reserved, blocks recovery)",
         st == 422, f"status={st}")

    # /lib/anything — same reservation via prefix.
    st, body = _try_activate("reserved-lib-sub", "['/lib/hijack']")
    test(f"{label} lib: route '/lib/hijack' rejected (reserved prefix)",
         st == 422, f"status={st}")

    # /bin — alias-strip confusion.
    st, body = _try_activate("reserved-bin", "['/bin']")
    test(f"{label} lib: route '/bin' rejected (reserved)",
         st == 422, f"status={st}")
    st, body = _try_activate("reserved-bin-sub", "['/bin/evil']")
    test(f"{label} lib: route '/bin/evil' rejected (reserved prefix)",
         st == 422, f"status={st}")

    # Route without leading slash.
    st, body = _try_activate("reserved-noslash", "['broken']")
    test(f"{label} lib: route without leading '/' rejected",
         st == 422, f"status={st}")

    # Inline core route shadow is ALLOWED (soft warn only) — activation
    # should succeed. We don't test the log output (stdout of subprocess),
    # just that the plugin lands active.
    st, body = _try_activate("shadow-share", "['/share']")
    test(f"{label} lib: shadowing /share is allowed (soft warn only)",
         st == 200, f"status={st} body={body[:120]}")
    # Clean up the allowed one.
    http_method(port, f"/lib/shadow-share", method="DELETE", token=approve)

    # Normal plugin route still works.
    st, body = _try_activate("normal-plug", "['/my-normal-route']")
    test(f"{label} lib: normal route /my-normal-route activates fine",
         st == 200, f"status={st}")
    http_method(port, "/lib/normal-plug", method="DELETE", token=approve)

    # Leftovers from rejected activations (state stayed pending, sources
    # still on disk). Clean them up so next test run starts fresh.
    for w in ("reserved-empty", "reserved-lib", "reserved-lib-sub",
              "reserved-bin", "reserved-bin-sub", "reserved-noslash"):
        http_method(port, f"/lib/{w}", method="DELETE", token=approve)
